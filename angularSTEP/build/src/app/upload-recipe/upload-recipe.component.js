...
let UploadRecipeComponent = class UploadRecipeComponent {
    ...
    onSubmit() {
        ...
        this.db.collection('recipes')
            .ref.withConverter(new recipe_converter_1.RecipeConverter().recipeConverter)
            .add(new recipe_1.Recipe(this.basicsFormGroup.value.name, '', this.basicsFormGroup.value.difficulty, this.basicsFormGroup.value.description, this.ingredientsArray.value, this.toolsArray.value, this.instructionsArray.value, this.extraFormGroup.value.extraInfo ? this.extraFormGroup.value.extraInfo : '', Date.now(), [], []));
