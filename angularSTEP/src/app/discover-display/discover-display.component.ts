...
export class DiscoverDisplayComponent implements OnInit {
  ...
  getUsers(){
    switch(this.sortType){
      case 'Time Created': {
        if(this.direction){
          this.userCollection.ref.orderBy("timestamp", "asc").withConverter(new Converter().userConverter).get().
          then((users) => {
            this.users = users.docs;
          });
        } else {
          this.userCollection.ref.orderBy("timestamp", "desc").withConverter(new Converter().userConverter).get().
          then((users) => {
            this.users = users.docs;
          });
        }
      ...
      case 'Number of Recipes': {
        ...
        if(this.direction){
          this.userCollection.ref.orderBy("displayName", "desc").withConverter(new Converter().userConverter).get().
          then((users) => {
            this.users = users.docs;
          });
        } else {
          this.userCollection.ref.orderBy("displayName", "asc").withConverter(new Converter().userConverter).get().
          then((users) => {
            this.users = users.docs;
          });
        }
